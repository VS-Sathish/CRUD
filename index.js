// const express = require("express"); // import the expree
...
// const MONGO_URL =
//   "mongodb+srv://sathish_01:sa01iso8h9@cluster0.9e9wptf.mongodb.net";
...
// since the connection process requires some time , we should give "await"

// this type of await declration is called "top-level await",  in which there is no necessary to declare the await with the async function..
...
app.get("/movies", async function (request, response) {
  ...
  const movies = await client
    ...
    .collection("movies")
    .find({})
    .toArray(); // find always returns "Cursor" => pagination
  response.send(movies);
});
...
// express.json() => middleware => used to convert the data to JSON
...
app.listen(PORT, () => console.log(`The server started in: ${PORT} ✨✨`)); // it is used to start the app
